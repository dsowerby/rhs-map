...
function addMarker(latitude, longitude, place, markerIconUrl) {
	...
	// header and link
	var markerContent = '<a href="'+place.url+'" target="_blank">'+place.name+'</a><br />';
	// directions
	...
	// description
	markerContent += place.text+'<br />';
	// image
	markerContent += '<img width="200px" src="'+place.image+'"/><br />';
...
function displayPlaces() {
	...
	for (var i = 0; i < placesData.length; i++) {
		...
		if (isPlaceDone(place)) {
			...
			addMarker(place.coordinate.latitude, place.coordinate.longitude, place);
		}
	}
}
